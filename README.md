...
- 新增「增强」设置标签页，模块化上下文增强：
  - 活动记忆：记录每日应用使用模式，为对话提供更丰富的上下文
  - 上下文搜索：根据当前窗口自动搜索相关信息（DuckDuckGo / 自定义 API）
  - 知识整理：LLM 自动整理搜索结果为简洁的知识摘要
  - 屏幕分析：视觉 API 从截图中提取关键词，增强上下文理解
  - 知识获取：基于屏幕分析主题词自动生成搜索任务，持续积累领域知识
- 上下文池架构：分层存储（短期/长期），基于 Jaccard 相似度的轻量 RAG 检索
- 主进程 Web 搜索 IPC：支持 DuckDuckGo HTML 抓取和自定义 API（Bing / SearXNG 等）
- 可调节回复长度倍率（×0.5 / ×1 / ×1.5 / ×2）
- 上下文数据自动脱敏，防止 API 密钥等敏感信息泄露
- 改进系统提示词：空闲提示不再假设有截图，改为基于上下文线索
- 情绪分类提示词完全国际化
- 自我认知提示词简化，不再包含具体外观描述
- 动态上下文移至角色规则之后，提示词结构更清晰
...
- VVM 下载后自动加入配置并重启 TTS
- VVM 保存按钮改为自动重启应用
- 修复 TTS 重启时 koffi 类型重复注册导致的失败
- 修复打包后 portable exe 的应用重启问题

### v1.6.0 — 系统托盘支持

- 新增系统托盘图标，应用最小化到托盘区域运行
- 启动宠物后设置窗口自动隐藏到托盘
- 左键点击托盘图标重新打开设置窗口
- 右键托盘菜单：显示设置 / 显示隐藏宠物 / 退出
- 关闭设置窗口不再退出应用，改为隐藏到托盘
...
- 新增设置界面 i18n 国际化支持（中文 / English / 日本語）
- 新增 UI 语言切换选项
- 更新内置角色卡，提供更具特色的代表性角色
- 内置角色卡版本自动同步，用户修改过的卡自动克隆保留
- 角色卡导入功能，支持文件选择和文件夹自动扫描
- 内置角色卡标记显示

### v1.4.0 — 翻译与对话优化

- 翻译 API 独立配置，与主 API 分离
- 翻译质量优化，API 超时时间增加
- 消息双缓冲机制，新增对话最小间隔设置
- AI 回复显示时过滤英文字母
- 设置页新增开源说明与 GitHub 链接

### v1.3.0 — 文档与体验优化

- 精简 API 配置说明，新增模型推荐（Grok / Gemini 3 Pro Preview）
- 完善 VOICEVOX 语音配置流程文档
- 新增问题排查指南与已知问题说明
- 新增贡献者与赞助者名单
- 开发者向内容折叠优化
...
- **内置角色卡**: 欢迎提交有趣的角色卡！内置角色卡需提供中/英/日三语版本。提交时需修改 `assets/prompts/<uuid>.json`（含 `i18n` 字段）和 `main.js` 中的 `ensureDefaultCharacters()`。格式参考现有内置卡
