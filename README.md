...
基于 Electron 的桌面宠物。Live2D 角色常驻桌面，通过截屏感知你的操作，AI 大模型生成陪伴式对话，VOICEVOX 语音合成实现语音输出。
...
- **VOICEVOX 语音** — 本地日语 TTS，中→日自动翻译，一键安装
...
看板需求: 由于版权原因本库不提供默认live2d模型。欢迎有意者提供可供分发的模型。出于同样的理由另外应用icon暂时由开发者头像占位。
