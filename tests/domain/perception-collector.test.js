...
//// 记录 push 的关键帧缓冲替身 [@x380kkm 2026-06-13] ////
function fakeBuffer() {
  const pushed = [];
  return { pushed, async push(frame) { pushed.push(frame); } };
}

//// 记录调用、按预置回态势与关键帧的抽取器替身 [@x380kkm 2026-06-13] ////
...
//// 记录 append 的记忆库替身 [@x380kkm 2026-06-13] ////
function fakeMemory() {
  const appended = [];
...
//// 一拍跑通:投帧、选帧、抽态势,非空态势写入记忆并返回 [@x380kkm 2026-06-13] ////
...
//// 抽取器选出关键帧时,态势抽取以最新关键帧为目标而非原始帧 [@x380kkm 2026-06-13] ////
...
//// 无关键帧选出时退回对原始帧抽态势 [@x380kkm 2026-06-13] ////
...
//// 态势为空时不写记忆并返回 null [@x380kkm 2026-06-13] ////
...
//// 空帧直接返回 null,不投帧不选帧不抽态势 [@x380kkm 2026-06-13] ////
...
//// 把抽态势的背景文本透传给抽取器 [@x380kkm 2026-06-13] ////
