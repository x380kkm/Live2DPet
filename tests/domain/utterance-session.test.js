...
//// 造一个记录发布事件的事件总线模拟 [@x380kkm 2026-06-13] ////
function makeBus() {
  const events = [];
  return { events, publish: (event) => events.push(event) };
}

//// 造一个把发言填上指定时长音频对齐的编排器模拟,可选合成期回调用于模拟竞态 [@x380kkm 2026-06-13] ////
...
//// 取消令牌缺省存活,取消后转为不活跃 [@x380kkm 2026-06-13] ////
...
//// 开始发言返回取消令牌并经总线发布产物 [@x380kkm 2026-06-13] ////
...
//// 无音频对齐时仍发布产物,气泡走默认时长 [@x380kkm 2026-06-13] ////
...
//// 新发言开始时取消上一条发言的令牌 [@x380kkm 2026-06-13] ////
...
//// 合成期间被新发言抢占的令牌不发布其产物 [@x380kkm 2026-06-13] ////
...
//// 取消当前发言会发布发言结束事件 [@x380kkm 2026-06-13] ////
...
//// 取消一个非当前的旧令牌只翻转其状态,不发结束事件 [@x380kkm 2026-06-13] ////
...
//// 缺事件总线时开始发言不抛错,仍返回令牌 [@x380kkm 2026-06-13] ////
