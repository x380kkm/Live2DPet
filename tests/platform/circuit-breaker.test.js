...
//// 闭合态下成功调用透传返回值并清零失败计数 [@busybee 2026-06-13] ////
test('成功调用返回结果且不断开', () => {
...
//// 连续失败达到上限后切断开态 [@busybee 2026-06-13] ////
test('连续失败到上限后断开', () => {
...
//// 失败但未达上限时仍保持闭合态 [@busybee 2026-06-13] ////
test('未达上限不断开', () => {
  const breaker = new CircuitBreaker({ maxFailures: 3 });
  const fail = () => { throw new Error('boom'); };
...
//// 失败后又成功会把失败计数清零,避免跨越式断开 [@busybee 2026-06-13] ////
test('中途成功清零失败计数', () => {
  const breaker = new CircuitBreaker({ maxFailures: 3 });
  const fail = () => { throw new Error('boom'); };
  breaker.execute(fail);
...
//// 断开态且未到重试时刻时直接返回降级值,不调用 operation [@busybee 2026-06-13] ////
test('断开态走降级且不执行操作', () => {
  let clock = 1000;
...
//// 断开后过了重试间隔则尝试恢复,放行下一次调用 [@busybee 2026-06-13] ////
test('过重试间隔后尝试恢复', () => {
  let clock = 1000;
...
//// 切断开与恢复时触发注入的通知回调 [@busybee 2026-06-13] ////
test('断开与恢复触发回调', () => {
  let clock = 0;
  let tripped = 0;
  let reset = 0;
