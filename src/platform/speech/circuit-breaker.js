...
//// 按连续失败计数在闭合态与断开态间切换的熔断器 [@x380kkm 2026-06-13] ////
class CircuitBreaker {
  ...
  //// 经熔断器执行一次调用,断开态下直接走降级 [@x380kkm 2026-06-13] ////
  execute(operation) {
  ...
  //// 当前是否处于断开态 [@x380kkm 2026-06-13] ////
  isOpen() {
    return this.open;
  }

  //// 断开态且未到重试时刻则拒绝,到时刻则转回闭合态尝试恢复 [@x380kkm 2026-06-13] ////
  _shouldReject() {
  ...
  //// 调用成功则清零失败计数 [@x380kkm 2026-06-13] ////
  _onSuccess() {
    this.failCount = 0;
  }

  //// 调用失败累加计数,达到上限切断开态 [@x380kkm 2026-06-13] ////
  _onFailure() {
    this.failCount++;
