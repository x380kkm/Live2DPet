...
// 气泡与 mod 前端各为独立窗口,排他由主进程一处协调。

//// 造表达区仲裁:按占用者名记当前主导,换主导时调被替下者的收起钩子 [@x380kkm 2026-06-14] ////
...
function createExpressionArbiter(deps) {
  ...
  return {
    //// 让一个占用者占主导:收起先前主导者,返回被替下的占用者名,已是它则不动 [@x380kkm 2026-06-14] ////
    takeOver(occupant) {
    ...
    //// 释放某占用者的主导:仅当它正占主导时回到无人占用 [@x380kkm 2026-06-14] ////
    release(occupant) {
      if (dominant === occupant) dominant = null;
    },
    //// 查当前占主导的占用者名,无人占用返回 null [@x380kkm 2026-06-14] ////
    current() {
      return dominant;
    }
  };
