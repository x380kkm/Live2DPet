...
//// 持有已注册处理器,按通道名查 [@x380kkm 2026-06-13] ////
const handlers = new Map();

//// 把一个未知或抛错的结果归一成调用方可判定的失败对象 [@x380kkm 2026-06-13] ////
function toFailure(error) {
  return { success: false, error: error && error.message ? error.message : String(error) };
}

//// 校验通道名属于契约目录,再登记处理器,重复登记同名通道报错 [@x380kkm 2026-06-13] ////
...
//// 按通道名取处理器并调用,收敛 try/catch:未注册或抛错都归一成失败对象 [@x380kkm 2026-06-13] ////
...
//// 判断一个通道是否已登记处理器,供组合根补齐剩余通道时避开重复注册 [@x380kkm 2026-06-13] ////
function isRegistered(channelName) {
  return handlers.has(channelName);
}

//// 判断一个目标窗口仍存活,集中替代各转发处逐字重复的存活判断 [@x380kkm 2026-06-13] ////
function isAlive(target) {
  return !!target && typeof target.isDestroyed === 'function' && !target.isDestroyed();
}

//// 清空已注册处理器,供测试在用例间复位 [@x380kkm 2026-06-13] ////
function reset() {
  handlers.clear();
}
