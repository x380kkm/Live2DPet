...
//// 组装路径工具:注入 app 与 path,封装打包态判定 [@busybee 2026-06-13] ////
function createPathUtils(app, path) {
  const isPackaged = app ? app.isPackaged : false;

  //// 算应用资源根目录:打包态取安装目录,开发态取项目根 [@busybee 2026-06-13] ////
  function assetsDir() {
    if (!app) return __dirname;
    return app.getAppPath();
  }

  //// 算用户数据目录:可写持久化数据的落盘根 [@busybee 2026-06-13] ////
  function userDataDir() {
    if (!app) return __dirname;
    return app.getPath('userData');
  }

  //// 把存储键映射成用户数据目录下的绝对文件路径 [@busybee 2026-06-13] ////
