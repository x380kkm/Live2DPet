...
const I18N = {
  en: {
    ...
    'routing.desc': 'Pick a model per category (Screenshot / Text / Translation). Each step follows its category by default; turn off "Follow category" to override it. Models and persona are yours to control; we do not review or endorse them, correctness is on you.',
    'routing.modeForm': 'Guided', 'routing.modeJson': 'JSON (advanced)',
    'routing.jsonLabel': 'Model config JSON (advanced; edit directly, invalid JSON is not saved)',
    'routing.injectionLabel': 'Extra system prompt (merged with the built-in prompt; not reviewed, your responsibility)',
    'routing.save': 'Save Routing',
    'mc.cat.vlm': 'Screenshot (vision-language model)', 'mc.cat.llm': 'Text (dialogue, routing, etc.)', 'mc.cat.translate': 'Translation',
    'mc.preset': 'Compatibility preset', 'mc.baseURL': 'Base URL', 'mc.apiKey': 'API Key', 'mc.model': 'Model name',
    'mc.steps': 'Per step (follows category by default, can override)', 'mc.follow': 'Follow category', 'mc.stepModel': 'Override model name', 'mc.temp': 'Temperature',
    'mc.step.keyframeSelect': 'Keyframe selection', 'mc.step.situationExtract': 'Situation extraction', 'mc.step.dialogue': 'Dialogue', 'mc.step.intentRoute': 'Intent & scene routing', 'mc.step.emotionSelect': 'Emotion selection', 'mc.step.reaction': 'Event reaction', 'mc.step.modGenerate': 'Mod generation', 'mc.step.translate': 'Translation',
    ...
    'prompt.hitActions': 'Interaction Descriptions',
    'prompt.hitActions.desc': 'Custom descriptions for each interaction type. Empty = default.',
    'prompt.hit.click.ph': 'clicked you',
    'prompt.hit.touch.ph': 'touched you',
    'prompt.hit.drag.ph': 'dragged you',
    'prompt.hit.swipe.ph': 'swiped past you',
    'prompt.hit.resize.ph': 'resized you',
    'prompt.save': 'Save',
    ...
    'sys.idle1': '(system: The user has been idle for a while, say something based on what you know or your own thoughts.)',
    'sys.idle2': '(system: Start a topic — could be about what the user has been doing, or your own thoughts.)',
    'sys.idle3': '(system: Share your thoughts or feelings about what the user has been up to.)',
    'sys.idle4': '(system: Share your current mood, or show care for the user.)',
    ...
    'sys.toneHint': '[Tone] Let your tone naturally reflect: {0}',
    'sys.windowUsage': '[Recent Window Usage] ',
    'sys.seconds': 's',
    'sys.screenshotAttached': '(with screenshot)',
    'sys.windowLayout': '[Desktop Layout (static, for reference only)] ',
    'sys.userIdle': '[User Input Idle] No keyboard/mouse activity for {0}s',
    'sys.petPosition': '[Your Position] Screen ({x},{y}), size {w}x{h} — that character in the screenshot is you.',
    'sys.historyScreenshot': '(User is using {0}, viewed screenshot)',
    'sys.historyUsing': '(User is using {0})',
    'sys.antiRepetition': '[Style Variation] Recent responses have repeated patterns ({0}). Use a different sentence structure, tone, and rhetorical approach this time.',
    'sys.patternQuestion': 'rhetorical questions',
    'sys.patternOpening': 'same opening words',
    'sys.patternEnding': 'same ending pattern',
    'sys.patternLength': 'similar response length',
    'sys.patternExclamation': 'exclamation overuse',
    'sys.patternEllipsis': 'ellipsis overuse',
    'sys.topicAvoid': '[Topic Avoidance] Recently discussed: {0}. Use a completely different topic or angle.',
    'sys.habitAvoid': '[Style Avoidance] Recent speech habits: {0}. Use different expression patterns.',
    'sys.analyzePrompt': 'Extract topics and speech habits from this character response. Output ONLY valid JSON: {"topics":["topic1","topic2"],"habits":["habit1"]}. topics: specific subjects mentioned (max 3, short phrases). habits: speech patterns like tone, rhetorical devices, sentence structure (max 2, short phrases). No explanation.',
    'sys.hitDefault.click': 'clicked you', 'sys.hitDefault.touch': 'touched you', 'sys.hitDefault.drag': 'dragged you', 'sys.hitDefault.swipe': 'swiped past you', 'sys.hitDefault.resize': 'resized you',
    'sys.hitContext': '(In the past minute: {0})',
    'sys.hitCount': '({0}) {1} time(s)',
    'sys.kfSelectPrompt': 'Select 2-3 most representative keyframes from N timestamped desktop screenshots. Criteria: different apps/content, significant content changes, avoid near-duplicates. If all frames look similar, select just 1. Output ONLY a JSON array of 0-based indices. Example: [0, 3, 5]',
    'sys.kfLabel': '[Visual Memory] Recent activity keyframes:',
    'sys.kfAck': '(understood)',
    'sys.kfEntry': 'Keyframe {0}: {1} - {2} ({3})',
    ...
    'sys.searchQueryPrompt': 'Extract 1-3 short search keywords from this window title. If the title is a local file path, IDE project, or contains no searchable content, return an empty array. Return only a JSON string array. Example: ["keyword1","keyword2"]',
  ...
  zh: {
    ...
    'routing.title': '模型自由装配',
    'routing.desc': '按大类(截图 / 文本 / 翻译)分别选模型;每个步骤默认跟随所属大类,关掉「跟随大类」即可单独换模型。模型与人设由你掌控,我们不审查、不背书,正确性你自负。',
    'routing.modeForm': '友好模式', 'routing.modeJson': 'JSON 高级',
    'routing.jsonLabel': '模型配置 JSON(高级;直接编辑,格式错误则不保存)',
    'routing.injectionLabel': '额外系统提示词(与出厂提示词合并;不审查,用户自负)',
    'routing.save': '保存模型配置',
    'mc.cat.vlm': '截图(视觉语言模型)', 'mc.cat.llm': '文本(台词、路由等)', 'mc.cat.translate': '翻译',
    'mc.preset': '兼容预设', 'mc.baseURL': '接口地址(baseURL)', 'mc.apiKey': '密钥(apiKey)', 'mc.model': '模型名',
    'mc.steps': '各步骤(默认跟随大类,可单独覆盖)', 'mc.follow': '跟随大类', 'mc.stepModel': '单独模型名', 'mc.temp': '温度',
    'mc.step.keyframeSelect': '关键帧选择', 'mc.step.situationExtract': '态势抽取', 'mc.step.dialogue': '台词生成', 'mc.step.intentRoute': '意图与场景路由', 'mc.step.emotionSelect': '情绪选择', 'mc.step.reaction': '事件反应', 'mc.step.modGenerate': 'mod 生成', 'mc.step.translate': '翻译',
    ...
    'prompt.hitActions': '互动描述',
    'prompt.hitActions.desc': '自定义每种互动类型的描述，留空则使用默认',
    'prompt.hit.click.ph': '点击了你',
    'prompt.hit.touch.ph': '摸了你',
    'prompt.hit.drag.ph': '拖拽了你',
    'prompt.hit.swipe.ph': '划过了你',
    'prompt.hit.resize.ph': '改变了你的大小',
    'prompt.save': '保存',
    ...
    'sys.idle1': '（system：用户一段时间没有操作了，说说你想说的吧，可以参考你知道的内容或者你自己的想法）',
    'sys.idle2': '（system：主动找个话题吧，可以是用户最近在做的事情，也可以是你自己的想法）',
    'sys.idle3': '（system：对用户最近在做的事情发表一下你的看法或感想吧）',
    'sys.idle4': '（system：分享一下此刻的心情，或者表达一下对用户的关心）',
    'sys.selfAwareness': '【自我认知】不要主动提及自己的外观或自己是Live2D角色。',
    'sys.emotionState': '【当前情绪状态】情绪值: ',
    'sys.emotionNext': '，下一个表情倾向: ',
    'sys.toneHint': '【语气】让语气自然地反映这个情绪: {0}',
    'sys.windowUsage': '【最近窗口使用情况】',
    'sys.seconds': '秒',
    'sys.screenshotAttached': '（附上屏幕截图）',
    'sys.windowLayout': '【桌面布局（静态参考信息）】',
    'sys.userIdle': '【用户输入空闲】已有{0}秒无键盘/鼠标操作',
    'sys.petPosition': '【你的位置】屏幕({x},{y})，大小{w}x{h}——截图中该位置的角色是你自己。',
    'sys.historyScreenshot': '（用户正在使用{0}，已查看截图）',
    'sys.historyUsing': '（用户正在使用{0}）',
    'sys.antiRepetition': '【表达变化】最近的回复出现了重复模式（{0}）。这次请使用不同的句式结构、语气和修辞手法。',
    'sys.patternQuestion': '反问句',
    'sys.patternOpening': '相同的开头',
    'sys.patternEnding': '相同的结尾',
    'sys.patternLength': '相似的回复长度',
    'sys.patternExclamation': '感叹号过多',
    'sys.patternEllipsis': '省略号过多',
    'sys.topicAvoid': '【话题回避】最近讨论过：{0}。请换个完全不同的话题或角度。',
    'sys.habitAvoid': '【语癖回避】最近的表达习惯：{0}。请使用不同的表达方式。',
    'sys.analyzePrompt': '从这段角色回复中提取话题和语癖。只输出合法JSON：{"topics":["话题1","话题2"],"habits":["语癖1"]}。topics：提到的具体话题（最多3个，短语）。habits：语气、修辞手法、句式结构等表达习惯（最多2个，短语）。不要解释。',
    'sys.hitDefault.click': '点击了你', 'sys.hitDefault.touch': '摸了你', 'sys.hitDefault.drag': '拖拽了你', 'sys.hitDefault.swipe': '划过了你', 'sys.hitDefault.resize': '改变了你的大小',
    'sys.hitContext': '(过去一分钟里{0})',
    'sys.hitCount': '({0}){1}次',
    'sys.kfSelectPrompt': '从N张带时间戳的桌面截图中选出2-3张最具代表性的关键帧。选择标准：不同应用/内容、内容变化显著、避免近似重复。如果所有帧相似，只选1张。只输出JSON数组（0-based索引）。示例：[0, 3, 5]',
    'sys.kfLabel': '[视觉记忆] 近期活动关键帧：',
    'sys.kfAck': '(understood)',
    'sys.kfEntry': '关键帧{0}: {1} - {2} ({3})',
    'sys.vlmPrompt': '从这张截图中提取3-5个关键词和一个简短的描述性标题。格式：keywords: k1, k2, k3 | title: 描述性标题。不要解释。用{0}输出。',
    'sys.vlmSituationPrompt': '你是一个上下文压缩器。重要：完全忽略屏幕上可见的桌宠、Live2D角色或浮动覆盖层——绝对不要提及它们。根据截图（可选附带上一张截图用于对比）、窗口标题和可选的背景数据，输出情景描述（最多800字符）。实体优先：优先识别和描述命名实体（角色名、游戏名、主播名、产品名、具体UI元素）及其状态（HP、分数、回合数、进度）。非实体的视觉描述（特效、颜色描述、布局位置）优先级低——除非对理解必要否则省略。对于游戏/直播/视频：输出结构化状态（如"第9回合，玩家A HP 5 vs 玩家B HP 12，场上4个随从，关键事件：造成5点伤害"）。确保数值准确。对于文字内容：引用屏幕上可见的关键文字。置信度规则：只有当截图中的文字清晰完整可读时才使用具体专有名词（卡牌名、技能名、物品名等）。对于小字、模糊或被遮挡的文字，改用画面描述（如"手牌8张"而非编造卡牌名称）。绝不捏造专有名词。"Previous"字段是之前AI提取的结果，可能包含编造的专有名词。始终根据当前截图验证实体名称——不要在没有清晰视觉确认的情况下从Previous中复制名称。避免模糊描述如"在浏览网页"——应描述显示了什么内容。如果提供了上次情景，集中于内容变化。如果确实没有任何变化，输出 UNCHANGED。用{0}输出。不要解释，只输出情景。',
    ...
    // 增强系统
    'enhance.master': '启用增强',
    'enhance.masterHint': '启用关键帧视觉记忆 — AI 可以看到近期活动的代表性截图（消耗额外 API tokens）',
    ...
    'sys.searchQueryPrompt': '从窗口标题中提取1-3个简短搜索关键词。如果标题是本地文件路径、IDE项目名或无可搜索内容，返回空数组。只返回JSON字符串数组。示例：["关键词1","关键词2"]',
  ...
  ja: {
    ...
    'routing.title': 'モデルルーティング',
    'routing.desc': 'カテゴリ(スクリーンショット / テキスト / 翻訳)ごとにモデルを選びます。各ステップは既定で所属カテゴリに従い、「カテゴリに従う」をオフにすると個別に変更できます。モデルと人設はあなたの管理下にあり、当方は審査も保証も行わず、正しさはご自身の責任です。',
    'routing.modeForm': 'かんたん', 'routing.modeJson': 'JSON(詳細)',
    'routing.jsonLabel': 'モデル設定 JSON(詳細;直接編集、書式エラーは保存しません)',
    'routing.injectionLabel': '追加のシステムプロンプト(出荷時プロンプトと統合;審査なし、自己責任)',
    'routing.save': 'ルーティングを保存',
    'mc.cat.vlm': 'スクリーンショット(視覚言語モデル)', 'mc.cat.llm': 'テキスト(セリフ・ルーティング等)', 'mc.cat.translate': '翻訳',
    'mc.preset': '互換プリセット', 'mc.baseURL': 'ベースURL', 'mc.apiKey': 'APIキー', 'mc.model': 'モデル名',
    'mc.steps': 'ステップ別(既定はカテゴリ追従、個別上書き可)', 'mc.follow': 'カテゴリに従う', 'mc.stepModel': '個別モデル名', 'mc.temp': '温度',
    'mc.step.keyframeSelect': 'キーフレーム選択', 'mc.step.situationExtract': '状況抽出', 'mc.step.dialogue': 'セリフ生成', 'mc.step.intentRoute': '意図・シーンルーティング', 'mc.step.emotionSelect': '感情選択', 'mc.step.reaction': 'イベント反応', 'mc.step.modGenerate': 'mod 生成', 'mc.step.translate': '翻訳',
    ...
    'prompt.hitActions': 'インタラクション説明',
    'prompt.hitActions.desc': '各インタラクションタイプのカスタム説明。空欄=デフォルト',
    'prompt.hit.click.ph': 'クリックした',
    'prompt.hit.touch.ph': '触った',
    'prompt.hit.drag.ph': 'ドラッグした',
    'prompt.hit.swipe.ph': 'スワイプした',
    'prompt.hit.resize.ph': 'サイズを変えた',
    'prompt.save': '保存',
    ...
    'sys.idle1': '（system：ユーザーがしばらく操作していません。知っていることや自分の考えから何か言ってみてください）',
    'sys.idle2': '（system：話題を見つけてください。ユーザーが最近していること、あなた自身の考えなど）',
    'sys.idle3': '（system：ユーザーが最近していることについて感想や意見を述べてください）',
    'sys.idle4': '（system：今の気持ちを共有するか、ユーザーへの関心を示してください）',
    'sys.selfAwareness': '【自己認識】自分の外見やLive2Dキャラクターであることを自発的に言及しないでください。',
    'sys.emotionState': '【現在の感情状態】感情値: ',
    'sys.emotionNext': '、次の表情傾向: ',
    'sys.toneHint': '【トーン】この感情を自然にトーンに反映させて: {0}',
    'sys.windowUsage': '【最近のウィンドウ使用状況】',
    'sys.seconds': '秒',
    'sys.screenshotAttached': '（スクリーンショット付き）',
    'sys.windowLayout': '【デスクトップレイアウト（静的参照情報）】',
    'sys.userIdle': '【ユーザー入力アイドル】{0}秒間キーボード/マウス操作なし',
    'sys.petPosition': '【あなたの位置】画面({x},{y})、サイズ{w}x{h}——スクリーンショットのその位置にいるキャラクターがあなたです。',
    'sys.historyScreenshot': '（ユーザーは{0}を使用中、スクリーンショットを確認済み）',
    'sys.historyUsing': '（ユーザーは{0}を使用中）',
    'sys.antiRepetition': '【表現の変化】最近の返答にパターンの繰り返しがあります（{0}）。今回は異なる文構造、トーン、修辞法を使ってください。',
    'sys.patternQuestion': '反語的質問',
    'sys.patternOpening': '同じ書き出し',
    'sys.patternEnding': '同じ結び方',
    'sys.patternLength': '似た応答長',
    'sys.patternExclamation': '感嘆符の多用',
    'sys.patternEllipsis': '省略記号の多用',
    'sys.topicAvoid': '【話題回避】最近の話題：{0}。全く異なる話題や視点に切り替えてください。',
    'sys.habitAvoid': '【口癖回避】最近の表現習慣：{0}。異なる表現パターンを使ってください。',
    'sys.analyzePrompt': 'このキャラクターの返答から話題と口癖を抽出。有効なJSONのみ出力：{"topics":["話題1","話題2"],"habits":["口癖1"]}。topics：言及された具体的な話題（最大3個、短いフレーズ）。habits：語調、修辞法、文構造などの表現習慣（最大2個、短いフレーズ）。説明不要。',
    'sys.hitDefault.click': 'クリックした', 'sys.hitDefault.touch': '触った', 'sys.hitDefault.drag': 'ドラッグした', 'sys.hitDefault.swipe': 'スワイプした', 'sys.hitDefault.resize': 'サイズを変えた',
    'sys.hitContext': '(過去1分間で{0})',
    'sys.hitCount': '({0}){1}回',
    'sys.kfSelectPrompt': 'N枚のタイムスタンプ付きデスクトップスクリーンショットから、最も代表的な2-3枚のキーフレームを選択。選択基準：異なるアプリ/コンテンツ、顕著なコンテンツ変化、類似フレームの回避。全フレームが類似の場合は1枚のみ。0-basedインデックスのJSON配列のみ出力。例：[0, 3, 5]',
    'sys.kfLabel': '[視覚メモリ] 最近のアクティビティキーフレーム：',
    'sys.kfAck': '(understood)',
    'sys.kfEntry': 'キーフレーム{0}: {1} - {2} ({3})',
    'sys.vlmPrompt': 'このスクリーンショットから3-5個のキーワードと短い説明的なタイトルを抽出してください。形式：keywords: k1, k2, k3 | title: 説明的なタイトル。説明不要。{0}で出力してください。',
    'sys.vlmSituationPrompt': 'コンテキスト圧縮器として。重要：画面上のデスクトップペット・Live2Dキャラクター・フローティングオーバーレイは完全に無視し、絶対に言及しないでください。スクリーンショット（オプションで前回も比較用に提供）・ウィンドウタイトル・オプションの背景データから、状況説明を出力（最大800文字）。エンティティ優先：命名エンティティ（キャラクター名、ゲームタイトル、配信者名、製品名、具体的なUI要素）とその状態（HP、スコア、ターン数、進行度）の識別と記述を優先。非エンティティの視覚的記述（エフェクト、色の説明、レイアウト位置）は優先度低——理解に不可欠でなければ省略。ゲーム/配信/動画の場合：構造化された状態を出力（例：「ターン9、プレイヤーA HP 5 vs プレイヤーB HP 12、場にミニオン4体、主要イベント：5ダメージ」）。数値の正確性を確保。テキストコンテンツの場合：画面上の重要なテキストを引用。信頼度ルール：スクリーンショット内のテキストが明確かつ完全に読み取れる場合のみ具体的な固有名詞（カード名、スキル名、アイテム名等）を使用。小さい・ぼやけた・一部隠れたテキストは視覚的に記述（例：「手札8枚」であってカード名を捏造しない）。固有名詞を捏造しないこと。「Previous」フィールドは以前のAI抽出結果であり、捏造された固有名詞を含む可能性があります。常に現在のスクリーンショットでエンティティ名を確認し、明確な視覚的確認なしにPreviousから名前をコピーしないでください。「ウェブを閲覧中」のような曖昧な記述は避け、表示されている内容を記述。前回の状況が提供された場合はコンテンツの変化に集中。本当に変化がなければ UNCHANGED と出力。{0}で出力。説明不要、状況のみ。',
    ...
    // 拡張システム
    'enhance.master': '拡張を有効化',
    'enhance.masterHint': 'キーフレーム視覚メモリを有効化 — AIが最近のアクティビティの代表的なスクリーンショットを参照（追加APIトークン消費）',
    ...
    'sys.searchQueryPrompt': 'ウィンドウタイトルから1-3個の短い検索キーワードを抽出。ローカルファイルパス・IDEプロジェクト名・検索不可能な内容の場合は空配列を返却。JSON文字列配列のみ返却。例：["キーワード1","キーワード2"]',
