...
//// 造一份空的两层模型配置骨架:三大类与步骤表加全局 system 注入 [@x380kkm 2026-06-13] ////
export function defaultModelConfig() {
  return { categories: {}, steps: {}, systemInjection: '' };
}

//// 把候选参数 id 排序成建议项在前、其余按字母序 [@x380kkm 2026-06-13] ////
...
//// 把秒转成毫秒整数,非正数视为缺省返回 null [@x380kkm 2026-06-13] ////
...
//// 把毫秒转成秒,缺失返回空串供输入框占位 [@x380kkm 2026-06-13] ////
export function msToSeconds(ms) {
  if (!ms) return '';
  return ms / 1000;
}

//// 取一个带正数下限兜底的秒转毫秒,用于缺省时长 [@x380kkm 2026-06-13] ////
export function secondsToMsWithFallback(seconds, fallbackMs) {
  return secondsToMs(seconds) || fallbackMs;
}

//// 把扫描出的图片列表与既有配置按文件名合并,保留既有的分类开关 [@x380kkm 2026-06-13] ////
...
//// 从图片的情绪名去重生成表情列表,供情绪系统使用 [@x380kkm 2026-06-13] ////
...
//// 把扫描出的表情结果映射成配置里的表情项 [@x380kkm 2026-06-13] ////
...
//// 把扫描出的分组动作扁平成动作情绪项,文件名去路径去后缀作名 [@x380kkm 2026-06-13] ////
...
//// 为每个 VVM 文件算出勾选面板的状态:磁盘是否就绪、是否选中、是否禁用 [@x380kkm 2026-06-13] ////
...
//// 在说话人元数据里按 styleId 定位说话人下标与该说话人的样式列表 [@x380kkm 2026-06-13] ////
...
//// 由倍率算出展示用的最大 token 数,基准 2048 [@x380kkm 2026-06-13] ////
export function tokenCountForMultiplier(multiplier) {
  return Math.round(2048 * multiplier);
}

//// 设置面板的配置数据模型:持有一份内存快照作真相来源,产出落盘用的补丁 [@x380kkm 2026-06-13] ////
export class SettingsModel {
  ...
  //// 取模型子配置的引用,缺失就地补建 [@x380kkm 2026-06-13] ////
  model() {
    if (!this.config.model) this.config.model = { type: 'none' };
    return this.config.model;
  }

  //// 记录一次模型扫描结果,供参数映射与列表面板渲染 [@x380kkm 2026-06-13] ////
  ...
  //// 把建议映射并入当前参数映射,空建议项跳过 [@x380kkm 2026-06-13] ////
  ...
  //// 设置某个语义参数映射到的模型参数 id,空值视为取消映射 [@x380kkm 2026-06-13] ////
  ...
  //// 用扫描出的图片列表合并刷新图片配置,保留既有分类开关 [@x380kkm 2026-06-13] ////
  ...
  //// 由图片情绪名重算表情列表,供保存图片模式时同步情绪系统 [@x380kkm 2026-06-13] ////
  ...
  //// 把面板收集到的表情与动作及缺省时长写回模型,返回启用的情绪名清单 [@x380kkm 2026-06-13] ////
  ...
  //// 取两层模型配置的引用,缺失就地补建空骨架 [@x380kkm 2026-06-13] ////
  modelConfig() {
