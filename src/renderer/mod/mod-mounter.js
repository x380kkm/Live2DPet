...
//// 判断一份前端规格是否为可执行档:kind 为 sandboxed [@x380kkm 2026-06-14] ////
export function isSandboxed(spec) {
  return !!spec && spec.kind === 'sandboxed';
}

//// 造受限 iframe 沙箱:只给 allow-scripts、srcdoc 载入前端,内部拿不到父窗口能力 [@x380kkm 2026-06-14] ////
...
//// 把纯数据声明模板渲成 DOM:标题、文本、按钮;按钮点击经 emit 产出交互事件 [@x380kkm 2026-06-14] ////
...
//// 校验一条来自沙箱的消息是否为白名单内的交互事件,合法则返回规整后的事件 [@x380kkm 2026-06-14] ////
...
//// 把一个 mod 挂到承载根:纯数据直渲、可执行走沙箱并桥接消息;返回卸载函数 [@x380kkm 2026-06-14] ////
