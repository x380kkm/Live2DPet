...
//// 表达一次发言取消状态的显式令牌 [@x380kkm 2026-06-13] ////
class CancelToken {
  constructor() {
    this.cancelled = false;
  }

  //// 标记为已取消 [@x380kkm 2026-06-13] ////
  cancel() {
    this.cancelled = true;
  }

  //// 是否仍在进行,即未被取消 [@x380kkm 2026-06-13] ////
  isActive() {
    return !this.cancelled;
  }
}

//// 协调一次发言的合成与对齐,产物经事件总线发布,取消经令牌 [@x380kkm 2026-06-13] ////
class UtteranceSession {
  ...
  //// 开始一次发言:取消上一条、合成对齐、发布产物,返回本次的取消令牌 [@x380kkm 2026-06-13] ////
  ...
  //// 经令牌取消进行中的发言,若取消的是当前发言则发布发言结束事件 [@x380kkm 2026-06-13] ////
  cancel(token) {
    if (!token) return;
    token.cancel();
  ...
  //// 把发言产物连同气泡时长与音频对齐经事件总线发布 [@x380kkm 2026-06-13] ////
  _publish(utterance) {
  ...
  //// 发布发言结束事件,供说话状态与气泡的订阅者收尾 [@x380kkm 2026-06-13] ////
  _publishEnded() {
    if (!this.eventBus) return;
