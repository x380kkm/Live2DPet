...
//// 缺省层级表:满、半、四分之一三层,各持一帧 [@x380kkm 2026-06-13] ////
...
//// 缺省按龄降采样档:越旧目标分辨率越低,超 maxAgeMs 淘汰 [@x380kkm 2026-06-13] ////
...
class KeyframeBuffer {
  //// 构造注入降采样函数、时钟、层级表与按龄档,装配各层的帧数组 [@x380kkm 2026-06-13] ////
  ...
  //// 写入一帧:落入层 0,逐层把溢出帧降采样下沉,最低层超额则丢最旧 [@x380kkm 2026-06-13] ////
  async push(frame) {
  ...
  //// 按年龄降采样后返回当前关键帧集:淘汰过旧帧,越旧分辨率越低,最新在前 [@x380kkm 2026-06-13] ////
  ...
  //// 按年龄在档位里查目标分辨率:取第一个年龄落入其窗的档 [@x380kkm 2026-06-13] ////
  ...
  //// 当前各层帧总数 [@x380kkm 2026-06-13] ////
  size() {
    return this.levels.reduce((sum, level) => sum + level.length, 0);
  }

  //// 清空缓冲:各层置空 [@x380kkm 2026-06-13] ////
  clear() {
