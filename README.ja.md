...
- 拡張モジュールは `window._enhanceLang` グローバル変数で言語を渡しています — 暗黙的な結合（現在のシングルウィンドウ構成では影響なし）
- `enhance-data.json` は全拡張データを単一ファイルに保存。将来的にレイヤー別分割や SQLite 移行を検討
...
- 新しい「拡張」設定タブ、モジュール式コンテキスト強化：
  - アクティビティ記憶：日々のアプリ使用パターンを記録し、会話コンテキストを充実
  - コンテキスト検索：現在のウィンドウに関連する情報を自動検索（DuckDuckGo / カスタム API）
  - 知識整理：LLM が検索結果を簡潔な知識要約に自動整理
  - 画面分析：ビジョン API がスクリーンショットからキーワードを抽出しコンテキストを強化
  - 知識獲得：画面分析のトピックから検索タスクを自動生成し、ドメイン知識を継続的に蓄積
- コンテキストプールアーキテクチャ：階層型ストレージ（短期/長期）、Jaccard 類似度ベースの軽量 RAG 検索
- メインプロセス Web 検索 IPC：DuckDuckGo HTML スクレイピングとカスタム API 対応（Bing / SearXNG 等）
- 応答長さ倍率の調整（×0.5 / ×1 / ×1.5 / ×2）
- コンテキストデータの自動サニタイズで API キーなどの機密情報漏洩を防止
- システムプロンプトの改善：アイドルプロンプトがスクリーンショット前提ではなくコンテキストベースに
- 感情分類プロンプトの完全国際化
- 自己認識プロンプトの簡素化、具体的な外見の記述を削除
- ダイナミックコンテキストをキャラクタールールの後に配置、プロンプト構造を改善
...
- ペットが画面座標情報でスクリーンショット内の自分を認識可能に
- ウィンドウタイトルの短縮でコンテキストをすっきり
- システムアイドル時間検出（キーボード/マウス無操作）
- 最小化ウィンドウのフィルタリング

### v1.7.0 — ウィンドウ認識強化 & GPU TTS

- ウィンドウ検出がウィンドウタイトル（ブラウザのタブ名など）を読み取り、タイトルごとに独立追跡
- AI リクエストにデスクトップのウィンドウレイアウト情報とウィンドウサイズを付加
- ワンクリックセットアップで DirectML (GPU) 版 ONNX Runtime も同時ダウンロード、GPU TTS に対応

<details>
<summary>過去のバージョン</summary>

### v1.6.1 — ホットリロードと自動再起動の改善

- モデル設定変更後、ペットウィンドウが自動でホットリロード（再起動不要）
- VVM ダウンロード後、自動で設定に追加し TTS を再起動
- VVM 保存ボタンでアプリを自動再起動
- koffi 型の重複登録による TTS 再起動失敗を修正
- ポータブル exe ビルドでのアプリ再起動を修正

### v1.6.0 — システムトレイ対応

- システムトレイアイコンを追加、アプリをトレイに最小化して実行
- ペット起動後、設定ウィンドウを自動的にトレイに非表示
- トレイアイコンを左クリックで設定ウィンドウを再表示
- トレイ右クリックメニュー：設定を表示 / ペットを表示・非表示 / 終了
- 設定ウィンドウを閉じてもアプリは終了せず、トレイに格納
